  ...
  quiz: [
    {
      ...
      options: ['"0" — a string containing the character zero', '0 — the number zero', '"" — the empty string'],
      correctIndex: 0,
      why: 'The falsy list contains the NUMBER 0 and the EMPTY string "" — but "0" is a non-empty string (one character aboard the train), so it’s truthy. The bin is decided by the list, not by looks.',
    },
    {
      question: 'let items = 0; if (items) { ship(); } else { askAgain(); } — the user legitimately ordered zero items. What happens?',
      options: [
        'ship() — 0 is a valid number',
        'askAgain() — 0 is falsy, so the code wrongly treats a real answer as “missing”',
        'An error',
      ],
      correctIndex: 1,
      why: 'The classic silent truthy/falsy bug: 0 is a perfectly meaningful quantity, but it’s falsy, so if (items) can’t tell “zero” from “nothing entered.” The fix is precision: if (items !== undefined). Test inputs like 0 and "" exist to smoke out exactly this.',
    },
    {
      question: 'How many falsy values are there (the everyday list)?',
      options: ['Six: false, 0, "", null, undefined, NaN', 'Dozens — it depends on context', 'Two: false and null'],
      correctIndex: 0,
      why: 'Just six, fixed by the language standard — everything else is truthy. Small enough to memorize, and worth it: this list is a favorite interview quickie and a tester’s edge-case checklist in disguise.',
    },
  ],
  teachBack: {
    prompt:
