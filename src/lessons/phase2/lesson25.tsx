...
export const lesson25: LessonDef = {
  ...
  quiz: [
    {
      question: 'let n = 10; while (n > 0) { n = n - 3; } — how many times does the body run, and what is n at the end?',
      options: ['3 times, n ends at 1', '4 times, n ends at -2', '10 times, n ends at 0'],
      correctIndex: 1,
      why: 'Trace the gate each lap: 10>0 run (n=7), 7>0 run (n=4), 4>0 run (n=1), 1>0 run (n=-2), then -2>0 is false → stop. Four laps, n = -2. Loops don’t stop AT the boundary — they stop when the check fails. Tracing like this is a daily testing skill.',
    },
    {
      question: 'A while loop’s condition is false the very first time it’s checked. What happens?',
      options: [
        'The body runs once, then stops',
        'The body never runs at all — zero iterations is perfectly legal',
        'An error — a loop must loop',
      ],
      correctIndex: 1,
      why: 'The gate comes BEFORE the road: false on arrival means the body is skipped entirely and life continues after the loop. Zero-iteration cases are classic test scenarios (empty cart, no results found).',
    },
    {
      question: 'Why does an infinite loop freeze the entire page, not just the loop?',
      options: [
        'It fills up memory until the tab crashes',
        'JavaScript has one single thread — while it’s trapped in the loop, nothing else (clicks, rendering) can run',
        'The browser pauses other code out of caution',
      ],
      correctIndex: 1,
      why: 'One worker, one racetrack, no exits: the single thread is busy doing exactly what you asked, forever. Clicks queue up unanswered, the page can’t even repaint. (Phase 6 turns this constraint into the async story.)',
    },
  ],
  teachBack: {
    prompt:
