...
export const lesson24: LessonDef = {
  ...
  quiz: [
    {
      question: 'let fee = age < 5 ? 0 : 100; — for age 3, what is fee?',
      options: ['0', '100', 'true'],
      correctIndex: 0,
    ...
    {
      question: 'let volume = 0; let setting = volume || 50; — the user deliberately chose volume 0. What is setting?',
      options: [
        '0 — their choice is respected',
        '50 — || saw 0 as falsy and “helpfully” replaced it. The ?? operator exists to fix exactly this',
        'An error',
      ],
      correctIndex: 1,
      why: '|| can’t tell “missing” from “legitimately zero” — 0 is falsy, so the default fires and stomps the user’s choice. volume ?? 50 would keep the 0 (?? only falls back on null/undefined). The 2.2 quantity bug, wearing a default-value costume.',
    },
    {
      question: 'user && user.name — what does && guarantee when user is null?',
      options: [
        'user.name is read carefully, returning null',
        'user.name is NEVER evaluated — short-circuit stops at the falsy left side, so no crash is possible',
        'It throws early, which is safer',
      ],
      correctIndex: 1,
      why: 'Short-circuit isn’t “ignores the result” — the right side never runs at all. That’s what makes && a guard: the dangerous read is unreachable when the left side is falsy. (Modern cousin: user?.name — Phase 8.)',
    },
  ],
  teachBack: {
    prompt:
