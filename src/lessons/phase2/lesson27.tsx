  ...
  quiz: [
    {
      ...
      options: [
        'break exits the whole loop; continue abandons only the current lap and keeps looping',
        'They’re synonyms with different spellings',
        'continue exits the loop; break pauses it',
      ],
      correctIndex: 0,
      why: 'break = ejector seat (loop over, done). continue = skip ramp (this lap is abandoned, the next check happens normally). Mixing them up inverts your loop’s logic while looking perfectly plausible — a classic code-review catch.',
    },
    {
      question: 'for (let i = 0; i < 100; i++) { if (found(i)) { break; } } — found() is true at i = 2. How many laps run?',
      options: ['100 — the loop always completes', '3 — laps for i = 0, 1, 2, then eject', '2'],
      correctIndex: 1,
      why: 'Laps run for i=0 (no), i=1 (no), i=2 (yes → break mid-lap). The remaining 97 laps never happen. This early-exit search is one of the most common loop patterns in existence.',
    },
    {
      question: 'Your test suite runs 25 test cases across 4 browsers using nested loops. The inner body runs how many times — and where does a break in the INNER loop land you?',
      options: [
        '29 times; break exits both loops',
        '100 times; break exits only the inner loop — you land in the outer lap, moving to the next browser',
        '100 times; break exits both loops',
      ],
      correctIndex: 1,
      why: '4 × 25 = 100 (nested loops multiply — the test-matrix shape). And break obeys the innermost-only rule: ejecting from the case-loop just advances to the next browser. To abandon everything you’d need a labeled break — or better, a function with return (Phase 3).',
    },
  ],
  teachBack: {
    prompt:
