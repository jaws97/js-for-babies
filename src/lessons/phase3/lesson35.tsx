...
export const lesson35: LessonDef = {
  ...
  quiz: [
    {
      question: 'A function’s body uses a variable declared OUTSIDE it, in the global bubble. Does that work?',
      options: [
        'No — functions only see their own variables',
        'Yes — the lookup ray travels outward through the wall and finds it',
        'Only if the variable is passed as an argument',
      ],
      correctIndex: 1,
      why: 'Inner sees outer — that’s the allowed direction. The ray leaves the function’s bubble, reaches global, finds the name. (Passing arguments is often cleaner style, but scope itself permits the outward look.)',
    },
    {
      question: 'Code in the global bubble tries to read a let variable declared inside a function. What happens?',
      options: [
        'It works — everything is connected',
        'ReferenceError — rays never travel inward, so the outside cannot see in',
        'undefined — the variable exists but is hidden',
      ],
      correctIndex: 1,
      why: 'The one-way glass. From outside, a function’s inner variables simply don’t exist — the lookup searches global, finds nothing, and throws ReferenceError. This privacy is a feature, and closures (lesson 3.7) will turn it into a superpower.',
    },
    {
      question: 'let x = "out"; function f() { let x = "in"; console.log(x); } — what does calling f() print?',
      options: ['"out" — the older variable wins', '"in" — the ray stops at the nearest match', 'An error — you can’t reuse the name x'],
      correctIndex: 1,
