...
export const lesson110: LessonDef = {
  ...
  quiz: [
    {
      question: 'What does 10 > 5 && 3 > 7 evaluate to?',
    ...
    {
      question: 'What does 2 + 3 * 4 give — and why?',
      options: [
        '20 — the machine reads left to right',
        '14 — * binds tighter, so it sits deeper in the tree and runs first',
        '24',
      ],
      correctIndex: 1,
      why: 'Precedence draws the tree: 3 * 4 is a subtree that resolves to 12, then 2 + 12 = 14. Left-to-right reading is a human habit; the machine climbs trees. Want 20? Write (2 + 3) * 4 — parentheses redraw the tree.',
    },
    {
      question: 'true || someScaryExpression — what can you say for certain?',
      options: [
        'The scary expression runs, then the result is true',
        'The result is true and the scary expression is NEVER evaluated — short-circuit',
        'It depends on the scary expression',
      ],
      correctIndex: 1,
      why: '|| stops the instant its left side is true — the answer is locked in, so the right side is skipped entirely. Real code leans on this deliberately (e.g. avoiding a lookup that would fail). Laziness as a feature.',
    },
  ],
  teachBack: {
    prompt:
